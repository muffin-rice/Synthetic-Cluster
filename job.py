...
class Job:
    ...
    def __init__(self, id : int, start_time : int, num_iterations : int, user_time_estimates : {int, int},
        ...
        self.user_time_ests = {k : v / SEC_PER_TICK for k,v in user_time_estimates.items()}
    ...
    def _perform_iteration(self):
        true_est_time = extrapolate_estimate(self.num_gpus, self.true_rates) / SEC_PER_TICK
        estimated_iters_per_tick = self.iter_total / true_est_time

        noise = ss.norm.rvs(0, .1)
        iters = estimated_iters_per_tick * (1+noise) #get the numbers of iters, with noise

        self.iter_completed += int(iters)


    def tick(self):
        self.time_elapsed += 1
        if self.num_gpus == 0 or self.finished:
            return
    ...
    def est_finish_time(self):
        '''estimated total time to finish with current estimates'''
        return extrapolate_estimate(self.BASE_GPU, self.profile_time_ests) + self.start_time

    @property
    def est_finish_time2(self):
        '''estimated finish time with current pace'''
        if self.iter_completed:
            return self.time_elapsed / self.progress
        
        return self.est_finish_time

    @property
    def fairness_ratio(self):
        if self.finished:
            return (self.finish_time - self.start_time) / (self.est_finish_time - self.start_time)
        elif self.iter_completed:
            return self.time_elapsed / self.progress
